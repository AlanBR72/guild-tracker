...
def last_online_requests(nome):
    try:
        ...
        # patterns robustos para dia/semana/mês/ano
        patterns = [
            (r'last online\s*[:\s]*?(\d+)\s*day',1),
            (r'last online\s*[:\s]*?(\d+)\s*days',1),
            (r'last online\s*[:\s]*?(\d+)\s*week',7),
            (r'last online\s*[:\s]*?(\d+)\s*weeks',7),
            (r'last online\s*[:\s]*?(\d+)\s*month',30),
            (r'last online\s*[:\s]*?(\d+)\s*months',30),
            (r'last online\s*[:\s]*?(\d+)\s*year',365),
            (r'last online\s*[:\s]*?(\d+)\s*years',365),
        ]
...
def analisar():
    ...
    # ThreadPoolExecutor para perfis realmente necessários
